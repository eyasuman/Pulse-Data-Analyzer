...
function LogEntry({ log, colors, isLast }: { log: AuditLog; colors: any; isLast: boolean }) {
  const meta = TYPE_META[log.type as LogType] ?? TYPE_META.System;
  ...
  return (
    <View style={entryStyles.wrapper}>
      <View style={entryStyles.left}>
        <View style={[entryStyles.dot, { backgroundColor: meta.color + "30", borderColor: meta.color }]} />
      ...
      <View style={[entryStyles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={entryStyles.cardHeader}>
          <View style={[entryStyles.typePill, { backgroundColor: meta.color + "15", borderColor: meta.color + "30" }]}>
...
const entryStyles = StyleSheet.create({
  ...
  time: { fontSize: 9, fontFamily: "Inter_400Regular" },
  actor: { fontSize: 13, fontWeight: "600", fontFamily: "Inter_600SemiBold" },
  action: { fontSize: 11, lineHeight: 16, fontFamily: "Inter_400Regular" },
