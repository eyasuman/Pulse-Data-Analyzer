...
function SummaryChip({ label, value, color, colors }: { label: string; value: string | number; color: string; colors: any }) {
  return (
    <View style={[summaryStyles.chip, { backgroundColor: color + "15", borderColor: color + "30" }]}>
...
const summaryStyles = StyleSheet.create({
  chip: { flex: 1, borderRadius: 10, borderWidth: 1, padding: 10, gap: 2 },
...
export default function AppointmentsScreen() {
  ...
  const totalRevenue = useMemo(
    () => appointments.filter((a) => a.status === "completed").reduce((acc, a) => acc + a.totalPrice, 0),
    [appointments]
  );
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.topBar, { paddingTop: topPt, backgroundColor: colors.background }]}>
        ...
        <View style={styles.summaryRow}>
          <SummaryChip label="Total" value={appointments.length} color={colors.primary} colors={colors} />
          <SummaryChip label="Revenue" value={`AED ${totalRevenue.toLocaleString()}`} color="#10b981" colors={colors} />
          <SummaryChip label="Pending" value={counts["pending"] ?? 0} color="#f59e0b" colors={colors} />
        </View>
        ...
        <View style={styles.filterRow}>
          {STATUS_FILTERS.map((f) => (
            <Pressable
              ...
              style={[
                styles.filterChip,
                { backgroundColor: activeFilter === f ? colors.primary : colors.card, borderColor: activeFilter === f ? colors.primary : colors.border },
              ]}
            >
...
const styles = StyleSheet.create({
  ...
  filterLabel: { fontSize: 11, fontWeight: "600", fontFamily: "Inter_600SemiBold", letterSpacing: 0.3 },
  filterCount: { fontSize: 10, fontFamily: "Inter_400Regular" },
