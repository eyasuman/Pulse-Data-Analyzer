  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (d) => d.name.toLowerCase().includes(q) || d.specialty.toLowerCase().includes(q) || d.city?.toLowerCase().includes(q)
      );
    }
    return result;
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <FlatList
        ...
        keyExtractor={(d) => d.id}
        renderItem={({ item }) => (
          <ProviderCard doctor={item} onPress={() => router.push(`/provider/${item.id}` as any)} />
        )}
