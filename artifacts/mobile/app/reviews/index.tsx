      ...
      <View style={[styles.topBar, { paddingTop: topPt, backgroundColor: colors.background }]}>
        ...
        <View style={styles.filterRow}>
          {STATUS_FILTERS.map((f) => (
            <Pressable key={f} onPress={() => setFilter(f)} style={[styles.chip, { backgroundColor: filter === f ? colors.primary : colors.card, borderColor: filter === f ? colors.primary : colors.border }]}>
              <Text style={[styles.chipLabel, { color: filter === f ? "#fff" : colors.mutedForeground }]}>
                {f === "all" ? `All ${counts.all}` : `${STATUS_META[f].label} ${counts[f] ?? 0}`}
              </Text>
            </Pressable>
          ))}
        </View>
...
function ReviewCard({ review, colors, onAction }: { review: Review; colors: any; onAction: () => void }) {
  const m = STATUS_META[review.status];
  const initials = review.patientName.split(" ").slice(0, 2).map((w: string) => w[0]).join("").toUpperCase();
  return (
    <View style={[cardStyles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={cardStyles.header}>
        <View style={[cardStyles.avatar, { backgroundColor: colors.primary + "20", borderColor: colors.primary + "30" }]}>
        ...
        <View style={cardStyles.headerInfo}>
          ...
          <Text style={[cardStyles.doctorName, { color: colors.mutedForeground }]}>{review.doctorName}</Text>
        </View>
...
const cardStyles = StyleSheet.create({
  ...
  initials: { fontSize: 13, fontWeight: "700", fontFamily: "Inter_700Bold" },
  headerInfo: { flex: 1 },
  patientName: { fontSize: 13, fontWeight: "600", fontFamily: "Inter_600SemiBold" },
  ...
  comment: { fontSize: 12, lineHeight: 18, fontFamily: "Inter_400Regular" },
});
...
const styles = StyleSheet.create({
  ...
  ratingText: { fontSize: 14, fontWeight: "700", fontFamily: "Inter_700Bold" },
  ...
  chipLabel: { fontSize: 11, fontWeight: "600", fontFamily: "Inter_600SemiBold" },
